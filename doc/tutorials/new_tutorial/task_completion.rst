...
.. _new_manual.task_completion:

######################################################################################################
|new_manual.task_completion.chapter_num|: Checkpointing: Interrupted Pipelines and Exceptions
######################################################################################################
...
***************************************
    ...
    In this case, the half-formed, incomplete output file will look newer than its inputs and hence up-to-date.
...
****************************************************************************
    ...
    The name of the checkpoint file can be set simply as a parameter to Ruffus functions:
    ...
    Some file systems do not appear to support SQLite at all. There are reports that SQLite databases have `file locking problems  <http://beets.radbox.org/blog/sqlite-nightmare.html>`_ on Lustre
    ...
    The name of checkpoint files for different pipelines can be specified in the environment variable ``DEFAULT_RUFFUS_HISTORY_FILE``::

        export DEFAULT_RUFFUS_HISTORY_FILE=/some/where/.ruffus_history.sqlite

    This gives considerable flexibility, and allows a system side policy to be set so that all Ruffus checkpoint files are set logically to particular paths:

    History file paths set directly via as a parameter to the function calls, or as a command line parameter to ``Ruffus.cmdline`` sensibly take precedence over the environmental variable.
...
******************************************************************************
    ...
    **Ruffus** supplements the file system time resolution but independently recording the timestamp at
    ...
    to network time slippage, misconfiguration etc, **Ruffus** reverts to using file system and adds a one second
    ...
    at the prices of some annoying 1s pauses, by setting ``pipeline_run(...., one_second_per_job = True,...)``
