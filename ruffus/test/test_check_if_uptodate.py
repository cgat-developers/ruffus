...
class Test_ruffus(unittest.TestCase):
    ...
    def test_ruffus (self):
        ...
        ret = s.getvalue()
        try:
            self.assertNotRegexpMatches(ret, r"Jobs needs update:\s*No function to check if up-to-date")
        except:
            print ("Oops: @check_if_uptodate is not being picked up\n")
            raise
        self.assertRegexpMatches(ret, r"Tasks which are up-to-date:(\n\s*)*Task = 'test_check_if_uptodate.task1'(\n\s*)*Task = 'test_check_if_uptodate.task2'")
